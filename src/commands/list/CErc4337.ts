...
export function CErc4337 () {
    return <ICommand>{
        ...
        description: [
            'ERC-4337: Account abstraction tools',
        ],
        subcommands: [

            {
                command: 'add',
                description: [
                    'Add existing Account Abstraction Account.'
                ],
                params: {
                    '-o, --owner': {
                ...
                async process(args: string[], params: { owner, address, name, chain }, app: App) {
                    ...
                    let accounts = await service.add(<Erc4337Account>{
                        type: 'erc4337',
                        name,
                        address,
            ...
            },
            {
                command: 'new',
                description: [
                    'Deploy new Account Abstraction Account.'
                ],
                params: {
                    '-n, --name': {
                    ...
                    '--contracts': {
                        description: 'Optionally, JSON serialized file with entryPoint, accountFactory contracts'
                    },
                    ...Parameters.pin({ required: true })
                },
                async process(args: string[], params: { name, owner, chain, contracts }, app: App) {
                    ...
                    $console.table([
                        ['ERC-4337 Address', gnosisSafe.safeAddress]
                    ]);
