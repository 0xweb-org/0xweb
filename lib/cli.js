...
exports.CHelp = {
    ...
    async process(args, params, app) {
        ...
        _console_1.$console.log('! We provide our Demo Keys for etherscan and co. Please, replace them with yours: bold<web3m config -e>!');
        _console_1.$console.log('');
        _console_1.$console.log(`web3m@${json.version} Commands`);
        _console_1.$console.log('');
...
exports.CInstall = {
    ...
    async process(args, params) {
        ...
        let output = atma_utils_1.class_Uri.combine(params.output ?? `./web3m/`, platform);
...
exports.CConfig = {
    ...
    async process(args, params, app) {
        if (params.edit) {
            ...
            _console_1.$console.log(`Open cyan<bold<${sysPath}>>`);
            Os.open(sysPath);
    ...
    subcommands: [
        {
            ...
            async process() {
                let configPathAccounts = _cli_1.$cli.getParamValue('config-accounts')
                    ?? atma_io_1.env.appdataDir.combine('./.web3m/accounts.json').toString();
...
class PackageService {
    ...
    async list() {
        let web3m = await atma_io_1.File.readAsync('web3m.json');
...
class InitWorker {
    ...
    async ensureDependencies() {
        ...
        _console_1.$console.toast(`Save new gray<package.json> with new deps: ${added}`);
